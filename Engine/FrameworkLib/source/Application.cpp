...
namespace NHTV
{
    ...
    Application::Application()
        : m_running( false )
    {
        assert( !g_App && "Only one application instance allowed." );
        g_App = this;
    }

    Application& Application::GetInstance()
    {
        assert( g_App && "Application has not been created." );
...
#pragma endregion
        ...
        // load default shader
        {
            ...
            std::string vertexShader = GetStringResource( "SHADER", DEFAULT_VERTEX_SHADER );
            std::string fragmentShader = GetStringResource( "SHADER", DEFAULT_FRAGMENT_SHADER );

            m_ShaderID = NHTV::Utility::loadShaderFromBuffer( 3, aszInputs, 2, aszOutputs, vertexShader.c_str(), fragmentShader.c_str() );
        ...
        {
            std::string vertexShader = GetStringResource( "SHADER", SPRITE_BATCH_VERTEX_SHADER );
            std::string geometryShader = GetStringResource( "SHADER", SPRITE_BATCH_GEOMETRY_SHADER );
            std::string fragmentShader = GetStringResource( "SHADER", DEFAULT_FRAGMENT_SHADER );

            m_SpriteBatchShaderID = NHTV::Utility::loadShaderFromBuffer( 0, nullptr, 0, nullptr, vertexShader.c_str(), fragmentShader.c_str(), geometryShader.c_str() );

            if ( m_SpriteBatchShaderID )
            {
                glUseProgram( m_SpriteBatchShaderID );
                // set the texture to use slot 0 in the shader
                GLuint texUniformID = glGetUniformLocation( m_SpriteBatchShaderID, "diffuseTexture" );
                glUniform1i( texUniformID, 0 );
    ...
    void Application::run()
    {
        ...
        do
        {
            ...
            {
                // Update shader parameters for the sprite batch
                glUseProgram( m_SpriteBatchShaderID );

                // set matrix uniforms within the shaders
                GLint ProjectionID = glGetUniformLocation( m_SpriteBatchShaderID, "Projection" );
                GLint ViewID = glGetUniformLocation( m_SpriteBatchShaderID, "View" );
    ...
    {
        FILE* fh = fopen( "../resources/appData.xml", "rb" );
        if ( fh != NULL )
        {
            TiXmlDocument doc( "../resources/appData.xml" );
            doc.LoadFile();
...
#pragma region Sprite creation and mainpulation functionality
    ...
    GLuint Application::GetSpriteBatchShader() const
    {
        return m_SpriteBatchShaderID;
    }
    ...
    unsigned int  Application::CreateSprite( const char* a_pTextureName, const NHTV::Rect& rect, const glm::vec2& a_origin, const glm::vec2& a_scale, const SColour& a_colour )
    {
        //Set up the colour
        glm::vec4 v4Color = glm::vec4( a_colour.argb.colours.r / 255.f,
                                       a_colour.argb.colours.g / 255.f,
                                       a_colour.argb.colours.b / 255.f,
                                       a_colour.argb.colours.a / 255.f );
        SpriteManager* pSpriteManager = SpriteManager::GetInstance();
        if ( pSpriteManager )
        {
            glm::vec2 v2Size = glm::vec2( rect.width * a_scale.x, rect.height * a_scale.y );
            return pSpriteManager->AddSprite( a_pTextureName, v2Size, a_origin, rect, v4Color );
        }
        return 0;
    }
    ...
    void Application::SetSpriteUVCoordinates( unsigned int a_uiSpriteID, const NHTV::Rect& rect )
    {
        SpriteManager* pSpriteManager = SpriteManager::GetInstance();
        if ( pSpriteManager )
        {
            pSpriteManager->SetSpriteUVCoordinates( a_uiSpriteID, rect );
        }
    }
...
#pragma region DrawString Functionality
    ...
    /**@{**/
    //////////////////////////////////////////////////////////////////////////
    void Application::DrawString( const char* a_pText, int a_iXPos, int a_iYPos, float a_fSize /*= 1.0f*/, SColour a_sColour /*= SColour(0xFF,0xFF,0xFF,0xFF)*/ )
    {
        ...
        {
            FontManager::GetInstance()->DrawString( v4Position, a_fSize, v4Colour, a_pText );
        }
    }
    /**@}**/
